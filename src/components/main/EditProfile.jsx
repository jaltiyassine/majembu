...
export default function EditProfile() {
  ...
  useEffect(() => {
    if (currentUser && currentUser.isSet) {
      const initialGenderIsOther = !['Male', 'Female', 'Prefer not to say'].includes(currentUser.gender);
      const genderValue = initialGenderIsOther ? 'Other' : currentUser.gender;
      const otherGenderValue = initialGenderIsOther ? currentUser.gender : '';

      setFormData({
        name: currentUser.name || '',
        age: currentUser.age ? String(currentUser.age) : '',
        gender: genderValue || '',
        otherGender: otherGenderValue || '',
  ...
  const handleSubmit = async (event) => {
    ...
    if (validate()) {
      ...
      const apiUrl = 'https://gen-hub.fun/update_profile.php';
      const requestBody = {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const updatedUserInfo = {
            ...
            description: formData.description,
            isSet: true,
            API_KEY: (responseData && responseData.API_KEY) ? responseData.API_KEY : currentUser.API_KEY,
          };

          dispatch(setInfo(updatedUserInfo));
          dispatch(addNotif({
          ...
          html.scrollTop = 0;
            ...
            const errorData = await response.json().catch(() => ({ message: "Profile update failed. Server error." }));
            dispatch(addNotif({
                id: Math.floor(100000 + Math.random() * 900000),
                severity: "error",
                message: errorData.message || `Profile update failed. Status: ${response.status}`,
                dateToShow: 1,
            }));
            const html = document.documentElement;
            html.scrollTop = 0;
  ...
  return (
    ...
    >
      ...
      <Button
        ...
        size="large"
        sx={{ mt: 2 }}
        disabled={isLoading || !currentUser || !currentUser.isSet}
      >
